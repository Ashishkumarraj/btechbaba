...
export default function Features() {
    ...
    const handleDownload = (e, chap, sub, semNumber) => {
        ...
        const fileData = {
            ...
            link: `/notes/sem${semNumber}/sub${sub}/chap.pdf`
        };
        ...
        const isDuplicate = existingDownloads.some(item => item.title === chap && item.subject === sub);
        if (!isDuplicate) {
            localStorage.setItem(userDownloadsKey, JSON.stringify([...existingDownloads, fileData]));
        }
    };
                                                                                            ...
                                                                                            {subData.chapters?.map((chap, cIdx) => (
                                                                                                <div key={cIdx} className="flex items-center justify-between bg-white dark:bg-gray-800 px-2.5 py-2 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 group hover:border-purple-200 dark:hover:border-purple-700 transition-colors">
                                                                                                    ...
                                                                                                    <a
                                                                                                        ...
                                                                                                        download
                                                                                                        className="flex-shrink-0 flex items-center justify-center bg-purple-100 dark:bg-purple-900/40 hover:bg-purple-500 hover:text-white text-purple-600 dark:text-purple-400 w-7 h-7 rounded-full transition-all group-hover:shadow-md"
                                                                                                        title={`Download ${chap.name} PDF`}
                                                                                                    >
                                                                                                        <FiDownload size={14} />
                                                                                                    </a>
                                                                                                </div>
