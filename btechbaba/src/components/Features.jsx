...
import { FiCheckCircle, FiChevronDown, FiChevronUp, FiDownload } from 'react-icons/fi';
...
export default function Features() {
    ...
    const handleDownload = (e, chap, sub, semNumber) => {
    ...
    return (
        <section id="notes" className="pt-24 pb-12 bg-gray-50 dark:bg-gray-900">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

                {/* Notes Section */}
                <div className="text-center max-w-2xl mx-auto mb-16">
                    <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">B.Tech Notes</h2>
                    <p className="text-gray-500 dark:text-gray-400 text-lg">Access highly structured PDF notes for all your semesters, subjects, and chapters.</p>
                </div>

                <div ref={sectionRef} className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
                    {curriculumData.map((yearData, yIdx) => (
                        <div key={yIdx} className={`reveal card-interactive${yIdx === 1 ? ' reveal-delay-1' : yIdx === 2 ? ' reveal-delay-2' : yIdx === 3 ? ' reveal-delay-3' : ''} bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden`}>
                            <button
                                onClick={(e) => { createRipple(e); setOpenYear(openYear === yIdx ? null : yIdx); setOpenSem(null); setOpenSubject(null); }}
                                className={`ripple-container w-full bg-white dark:bg-gray-800 hover:bg-purple-50 dark:hover:bg-purple-900/30 transition-colors ${openYear === yIdx ? 'px-5 py-5 flex items-center justify-between border-b border-gray-100 dark:border-gray-700' : 'aspect-square flex flex-col items-center justify-center p-6'}`}
                            >
                                {openYear !== yIdx ? (
                                    <>
                                                                                    ...
                                                                                    <div className="overflow-hidden">
                                                                                        <div className="bg-gray-50 dark:bg-gray-900 p-2 space-y-2 border-t border-gray-200 dark:border-gray-700">
                                                                                            {subData.chapters?.map((chap, cIdx) => {
                                                                                                const link = (chap.id && chap.id.startsWith('http')) ? chap.id : ((pdfUrls?.[yearData.year]?.[semData.semesterLabel]?.[subData.name]?.[chap.name]) || '#');
                                                                                                return (
                                                                                                    <div key={cIdx} className="flex items-center justify-between bg-white dark:bg-gray-800 px-2.5 py-2 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 group hover:border-purple-200 dark:hover:border-purple-700 transition-colors">
                                                                                                        <span className="text-xs font-medium text-gray-600 dark:text-gray-400 line-clamp-1 flex-1 pr-2 group-hover:text-gray-900 dark:hover:text-white transition-colors" title={chap.name}>{chap.name}</span>
                                                                                                        <a
                                                                                                            href={link}
                                                                                                            {...(link.startsWith('http') ? { target: "_blank", rel: "noopener noreferrer" } : { download: true })}
                                                                                                            onClick={(e) => handleDownload(e, chap.name, subData.name, actualSemNumber)}
                                                                                                            className={`btn-download flex-shrink-0 flex items-center justify-center w-7 h-7 rounded-full group-hover:shadow-md ${link !== '#' ? 'bg-purple-100 dark:bg-purple-900/40 hover:bg-purple-500 hover:text-white text-purple-600 dark:text-purple-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 cursor-not-allowed'}`}
                                                                                                            title={link !== '#' ? `Download ${chap.name} PDF` : `Not Available`}
                                                                                                        >
                                                                                                            <FiDownload size={14} />
                                                                                                        </a>
                                                                                                    </div>
